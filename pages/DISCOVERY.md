...
3. **Deploy mechanism could not be confirmed from here.** No `wrangler.toml`, `wrangler.jsonc`, `functions/`, `_redirects`, `_headers`, `.github/workflows/` or `package.json` exist. That is consistent with git-connected Cloudflare Pages deploying `main` with no build command, which is the assumption `deploy-page.js` codes to. The sandbox's egress policy blocks `ieatzhealthy.com`, `ieatshealthy.com`, `apps.apple.com` and `itunes.apple.com` (proxy returns 403 on CONNECT), so gate 1 ("a trivial commit deploys and is visible") and every `verify-live.js` poll must run from Travis's machine or a Cowork task with network. **Travis: confirm in the Cloudflare dashboard that the Pages project is connected to `travi-trav3/iEatz`, production branch `main`.**
   - Resolved 2026-09-12 15:32Z: the project's production branch was `claude/brave-faraday-o5q71u`, not `main`. Switched to `main` by the verify-recipes Action's `cf_action=set-production-branch` step, which uses the `CLOUDFLARE_API_TOKEN` repo secret (Cloudflare Pages Edit, iEatz account). From now on `cf_action=inspect` shows the production branch, domains and the environment of every recent deployment, and `cf_action=deploy` creates a production deployment, all from the runner. Page two verified live one minute later.
   - Update 2026-09-12: confirmed git-connected. The project is `ieatz` on account `5ad6af2a...` and posts a "Cloudflare Pages" check run on every commit to main with the deployment's pages.dev URL (`<hash>.ieatz.pages.dev`, branch alias `main.ieatz.pages.dev`, production `ieatz.pages.dev`). Builds take about 20 seconds. But production stopped following main after the page one merge (4dd1988, 2026-09-09): later pushes build as previews only, and `ieatzhealthy.com` still serves the 09-09 deployment. See `runs/run-005-page-two.md`. The verify-recipes Action has a `probe_urls` dispatch input for exactly this kind of diagnosis, because neither the sandbox nor the Cloudflare connector (no Pages tools) can see the site.
   - `www.ieatzhealthy.com` answers Cloudflare error 525 (SSL handshake failed). Pages and pins only ever use the apex, but any traffic that lands on www is lost until the DNS or origin certificate for www is fixed.
...
7. **GitHub Actions on the website repo: enabled 2026-09-12 (was disabled at build time).** The repo policy allows only first-party workflows, so `verify-recipes.yml` uses no marketplace actions (plain git clone with the job token, Node from the runner image). First real run 34672839503 verified page one live in one attempt. Original finding kept for the record: `.github/workflows/verify-recipes.yml` is on `main` (ed7e7a8) but the API lists zero workflows and `workflow_dispatch` returns 404. Until Actions is enabled in the repo settings, no machine in this environment can confirm a page is live, and pins for new pages stay Buffer drafts. Enabling it is a one-time setting; after that the Action runs on every push to `main` that touches `recipes/`.
