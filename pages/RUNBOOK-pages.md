...
The existing render harness produces the pin creative when it is available. When it is not (a remote session), `lib/pin/render-pin.js` renders the 1000x1500 creative from the page's headline and hero photo with the self-hosted brand fonts; commit it under `posts/pages/<id>.png`, pin the raw.githubusercontent URL to the commit SHA, and curl it for 200 before scheduling. Buffer: `create_post` on the Pinterest channel with `metadata.pinterest = {boardServiceId, title, url}`, `mode: customScheduled`, `dueAt` 7 or more days out, then the Slack alert in **#social** (`config.slack.channel_id`, C0BATGA438T) at `dueAt`. Never #all-ieatz-healthy: that is the company-wide channel, and alerts posted there through 2026-09-21 had to be moved.

**Alert message (`config.slack.message`).** Three lines: platform and title, the two links, the engagement ask. Every URL is a markdown link `[label](url)`, never bare: the Slack connector autolinks a bare URL and swallows the line break after it, so the next word is absorbed into the link and the link no longer opens (the 2026-09-18 alert shipped this way). Keep both links on one line separated by ` · `. The page link is the clean URL without UTMs so the team's clicks are not counted as Pinterest traffic. Record the returned scheduled-message id as `pin.slack_alert.scheduled_message_id` with `channel` set to the config value; a rescheduled pin needs a new alert, and the old scheduled message has to be deleted by hand in Slack (Drafts & sent > Scheduled), since the connector cannot delete one.
